    ...
    class Builder : IOBase {
        ...
        public Result Build() {
            ...
            if (parameters.modFolderName == null || parameters.modFolderName == "")
                return Result.NoModName;
            if (parameters.wiimmfiRegion == -1)
                return Result.NoWiimmfi;
            ...
            try {
                    ...
                    if (buildParams == BuildParams.Full) {
                        ...
                        for (int i = 0; i < ctsCupCount; i++) {
                            ...
                            if (i >= 100 || cup.iconName != $"{MainWindow.Cup.defaultNames[i]}.png") {
                                ...
                                if (i >= 100)
                                    ++extraIcons;
                            } else if (cup.iconName == "")
                                customCupIcons.Add((MainWindow.Cup.defaultNames[i], i));
                        }
                        if (hasCustomIcons) {
                            ...
                            File.Copy("temp/UIAssets.szs", $"{modFolder}/Assets/UIAssets.szs");
                    }

                    string gameFolderName = $"/{parameters.modFolderName}";
                    if (buildParams == BuildParams.ConfigOnly)
                        configHeader.version *= -1;
                ...
                if (Directory.Exists(finalDirName)) {
                    try {
                        Directory.Delete(finalDirName, true);
                    } catch {
                        return Result.AlreadyInUse;
                    }
                }
        ...
        private Result WriteMainTrack(MainWindow.Cup.Track track, uint idx, string[] expertFileNames, bool isFake) {
            ...
            for (int j = 0; j < track.variants.Count; j++) {
                ...
                uint vIdx = (uint)j + 1;
                ret = WriteVariant(variant, idx, vIdx, variant.expertFileNames, isFake, true);
                if (ret != Result.Success)
                    return ret;
