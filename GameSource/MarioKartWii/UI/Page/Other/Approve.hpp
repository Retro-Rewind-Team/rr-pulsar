...
class Approve : public Page {  // ID 0x52
    ...
    void SetTitleMessage(u32 bmgId, const Text::Info *text = nullptr);  // 805c21a8
    void SetMessageWindowMessage(u32 bmgId, const Text::Info *text = nullptr);  // 805c21b0
    void SetApproveButtonMessage(u32 bmgId, const Text::Info *text = nullptr);  // 805c21b8
    ...
    const PtmfHolder_2A<Page, void, Approve &, float> *onApproveHandler;  // 0x820 page refers to another page here, this is triggered OnButtonClick
    const PtmfHolder_2A<Page, void, Approve &, float> *onDisapproveHandler;  // 0x824 same but for back functions, float = delay
