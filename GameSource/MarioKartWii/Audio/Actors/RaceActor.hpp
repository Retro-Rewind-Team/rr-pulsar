...
class Actor : public EGG::Audio3DActor {  // value for <4>||<2>||<1>
   public:
    ...
    virtual Handle* TryStartSound(u32 soundId);  // 0x18 807017f8||80706364||8070578c
    virtual Handle* TryStopAndStartSound(u32 soundId, u32 handleIdToStop);  // 0x1c 807019d0||8070653c||80705964 stops handleToStop sound and starts a new one
    virtual Handle* TryStartSound(ul soundId);  // 0x20 80701884||807063f0||80705818
    virtual Handle* TryStopAndStartSound(ul soundId, u32 handleIdToStop);  // 0x24 80701a64||807065d0||807059f8
    virtual Handle* TryStartSound(const char* soundName);  // 0x28 80701910||8070647c||807058a4
    virtual Handle* TryStopAndStartSound(const char* soundName);  // 0x2c 80701af8||80706664||80705a8c stops current sound and starts a new one
    virtual Handle* TryHoldSound(u32 soundId);  // 0x30 80701bb4||80706720||80705b48
    virtual Handle* TryHoldSound(u32 soundId, u32 handleId);  // 0x34 80701ea0||80706a0c||80705e34
    virtual Handle* TryHoldSound(ul soundId);  // 0x38 80701fc9c||80706808||80705c30
    virtual Handle* TryHoldSound(ul soundId, u32 handleId);  // 0x3c 80701f08||80706a74||80705e9c
    virtual Handle* TryHoldSound(const char* soundName);  // 0x40 80701d84||807068f0||80705d18
    virtual Handle* TryHoldSound(const char* soundName, u32 handleId);  // 0x44 80701f70||80706adc||80705f04
    virtual Handle* TryPrepareSound(u32 soundId);  // 0x48 80702014||80706b80||80705fa8
    virtual Handle* TryStopAndPrepareSound(u32 soundId, u32 handleIdToStop);  // 0x4c 807021ec||80706d58||80706180
    virtual Handle* TryPrepareSound(ul soundId);  // 0x50 807020a0||80706c0c||80706034
    virtual Handle* TryStopAndPrepareSound(ul soundId, u32 handleIdToStop);  // 0x54 80702280||80706dec||80706214
    virtual Handle* TryPrepareSound(const char* soundName);  // 0x58 8070212c||80706c98||807060c0
    virtual Handle* TryStopAndPrepareSound(const char* soundName, u32 handleIdTopStop);  // 0x5c 80702314||80706e80||807062a8
    Handle handles[handleNum];  // 0x80
...
class LinkedRaceActor {  // actors managed by RaceAudioManager //nw4r has a similar "LinkedObject" so names copied
   public:
    ...
    virtual Handle* StartSoundLimited(u32 soundId, float localVar) = 0;  // 0x30 checks for conditions before starting
    virtual Handle* HoldSoundLimited(u32 soundId) = 0;  // 0x34
    virtual void StopAllSound(int fadeOutFrames) = 0;  // 0x38
    virtual Handle* StartNewSoundLimited(u32 soundId, float volume) = 0;  // 0x3c no overlap; if an handle is already set, does not start the sound
    virtual bool StartSound(u32 soundId);  // 40 807013a0
    virtual bool HoldSound(u32 soundId);  // 44 80701398 returns false as this has no Handle natively
    virtual void StopSound(int fadeOutFrames);  // 0x48 80701394 just a blr as this has no Handle natively
    virtual bool StartSound(u32 soundId, Handle* handle) = 0;  // 0x4c
    virtual bool StartSound(u32 soundId, Handle* handle, const snd::SoundStartable::StartInfo* startInfo) = 0;  // 0x50
    virtual bool HoldSound(u32 soundId, Handle* handle) = 0;  // 0x54
    virtual bool HoldSound(u32 soundId, Handle* handle, const snd::SoundStartable::StartInfo* startInfo) = 0;  // 0x58
    ...
    u8 userData;  // 0x1e for example in kartActor, KCLVariant
...
class RaceActor : public Actor<4>, public LinkedRaceActor {
   public:
    ...
    Handle* StartSoundLimited(u32 soundId, float localVar) override;  // thunk 80702638 func 80700d4c
    Handle* HoldSoundLimited(u32 soundId) override;  // thunk 80702630 func 807014e8
...
class SimpleRaceActor : public RaceActor {  // for actors without BRASD support
   public:
    ...
    ~SimpleRaceActor() override;  // 80707360
...
class Anim : public snd::detail::AnimSoundImpl {  // BRASD for example penguins
...
class RaceAnimActor : public RaceActor {
   public:
    ...
    ~RaceAnimActor() override;  // 80702498
