...
struct Quatf : nw4r::math::QUAT {
    ...
    void RotateVector(const EGG::Vector3f &point, EGG::Vector3f &dest);  // 8023a2d0 rotates quat by vec and then converts it to a vec
    void RotateVectorConjugate(const EGG::Vector3f &point, EGG::Vector3f &dest);  // 8023a404
    void MultiplyVector(const Quatf &quat, const EGG::Vector3f &point, Quatf &dest);  // 8023a540 no clue why the syntax is so weird
    void SlerpTo(const Quatf &q2, Quatf &dest, float parameter) const;  // 8023a5c4 interpolates between two angles with percent
