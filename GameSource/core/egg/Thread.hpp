...
public:
    ...
    Heap *heap;
    OS::Thread *osThread;  // 0x8
    OS::MessageQueue messageQueue;
    OS::Message *messagesArray;  // msgcount messages
    u32 msgCount;
    ...
    static nw4r::ut::List threadList;  // 80384b60
};  // total size 0x48
...
public:
    ...
    struct TJob {  // this is the message for OSSendMessage
        ...
        void *args;
        OS::Message *endMessage;  // 0x8 at the end of run, if not null, gets sent
    ...
    template <class T>
    inline bool Request(void (*function)(T), T args, u32 r6) {
        return Request(reinterpret_cast<TaskFunction>(function), reinterpret_cast<void *>(args), r6);
    }
