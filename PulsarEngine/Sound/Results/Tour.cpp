#include <kamek.hpp>
#include <core/nw4r/ut/BinaryFileFormat.hpp>
#include <core/nw4r/snd/DVDSoundArchive.hpp>
#include <SlotExpansion/CupsConfig.hpp>

namespace Pulsar {
namespace Sound {

static char tourFanfarePath[0x100];

static bool StringsEqual(const char* lhs, const char* rhs) {
    if (lhs == nullptr || rhs == nullptr) return false;
    while (*lhs != '\0' && *rhs != '\0') {
        if (*lhs != *rhs) return false;
        ++lhs;
        ++rhs;
    }
    return *lhs == *rhs;
}

static bool IsTourResultTrackName(const char* fileName) {
    if (fileName == nullptr) return false;

    static const char* const names[] = {
        "tVV", "tSingS", "tAD", "tMD", "rMC1", "rCI1", "rCI2",
        "rRR1", "rRR2", "rVL1", "rVL2", "rGV1", "rBC1", "rDP1"
    };
    for (u32 i = 0; i < sizeof(names) / sizeof(names[0]); ++i) {
        if (StringsEqual(fileName, names[i])) return true;

        char nameWithExtension[16];
        snprintf(nameWithExtension, sizeof(nameWithExtension), "%s.szs", names[i]);
        if (StringsEqual(fileName, nameWithExtension)) return true;
    }
    return false;
}

static bool IsTourResultTrack() {
    const CupsConfig* cupsConfig = CupsConfig::sInstance;
    if (cupsConfig == nullptr) return false;

    const PulsarId track = cupsConfig->GetWinning();
    if (CupsConfig::IsReg(track)) return false;

    const u8 variantIdx = cupsConfig->GetCurVariantIdx();
    const char* fileName = cupsConfig->GetFileName(track, variantIdx);
    if (fileName == nullptr || fileName[0] == '\0') fileName = cupsConfig->GetFileName(track, 0);
    return IsTourResultTrackName(fileName);
}

static const char* GetTourFanfareName(const char* extFilePath) {
    if (extFilePath == nullptr) return nullptr;

    const char* fileName = extFilePath;
    for (const char* cursor = extFilePath; *cursor != '\0'; ++cursor) {
        if (*cursor == '/') fileName = cursor + 1;
    }

    if (StringsEqual(fileName, "o_FanfareGP1_32.brstm")) return "o_FanfareTourGP1_32.brstm";
    if (StringsEqual(fileName, "o_FanfareGP2_32.brstm")) return "o_FanfareTourGP2_32.brstm";
    if (StringsEqual(fileName, "o_FanfareGPdame_32.brstm")) return "o_FanfareTourGPdame_32.brstm";
    return nullptr;
}

bool ResolveTourFanfarePath(const nw4r::snd::DVDSoundArchive* archive, const char*& extFilePath) {
    const char* tourFanfare = GetTourFanfareName(extFilePath);
    if (archive == nullptr || tourFanfare == nullptr || !IsTourResultTrack()) return false;

    snprintf(tourFanfarePath, sizeof(tourFanfarePath), "%sstrm/%s", archive->extFileRoot, tourFanfare);
    extFilePath = tourFanfarePath;
    return true;
}

}  // namespace Sound
}  // namespace Pulsar
