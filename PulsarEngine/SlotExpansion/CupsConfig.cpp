...
CupsConfig::CupsConfig(const CupsHolder& rawCups) : regsMode(rawCups.regsMode),
    ...
    for (int i = 0; i < 88; ++i) {
        alphabeticalArray[168 + i] = lastTrackIndices[i];
    }
    for (int i = 168 + 88; i < ctsCount; ++i) {
        alphabeticalArray[i] = i;
    }
