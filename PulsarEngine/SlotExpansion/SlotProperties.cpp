...
KartType PreventRSLCrash(Effects::Player *effects, const Kart::Link &link) {
    bool isSherbet = true;
    if (Racedata::sInstance->racesScenario.settings.courseId != N64_SHERBET_LAND || ArchiveMgr::sInstance->GetFile(ARCHIVE_HOLDER_COURSE, "ice.brres", 0) == nullptr) isSherbet = false;
...
static void *PreventMHCrash(u32 carManagerSize) {
    ...
    if (root->GetFile(ARCHIVE_HOLDER_COURSE, "K_car_body.brres") == nullptr || root->GetFile(ARCHIVE_HOLDER_COURSE, "K_truck.brres") == nullptr) return nullptr;
...
static bool PreventMHMatCrash(const Racedata &racedata) {
    ...
    for (int i = 0; i < 12; ++i)
        if (mdl.GetResMat(MHModelDirector::matNames[i]).data == nullptr) return false;
    return true;
}
