    ...
    if (mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE)
        ...
        if (mode >= MODE_PRIVATE_VS && mode <= MODE_PRIVATE_BATTLE) {
            ...
            if (sub.localAid == hostAid)
                seed = scenario.settings.selectId;
            else
                seed = controller->GetReceivedPacketHolder<RKNet::RACEHEADER1Packet>(hostAid)->packet->selectId;
        } else {
            u32 tick = OS::GetTick();
    ...
    if (val < 0x1e00) {
        ...
        switch (val) {
            case 0x1000:
                ...
                break;
            case 0x1002:
                ret = localPlayerCount == 1;
                break;
            case 0x102e:
                ret = localPlayerCount == 2;
                break;
            case 0x1030:
                ret = localPlayerCount == 3;
                break;
            case 0x1032:
                ret = localPlayerCount == 4;
                break;
            case 0x1034:
                ret = localPlayerCount < 3;
                break;
            case 0x1036:
                ret = localPlayerCount & 0x1;
                break;
            case 0x1038:
                ret = (localPlayerCount == 1 || localPlayerCount == 4);
                break;
            ...
            case 0x1018:
                ...
                break;  // 13 - 18
            case 0x1004:
                ret = mode <= MODE_6 || mode >= MODE_AWARD;
                break;
            case 0x101a:
                ret = mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE;
                break;
            case 0x101c:
                ret = isBattle;
                break;
            case 0x101e:
                ret = isBattle && scenario.settings.battleType == BATTLE_BALLOON;
                break;
            case 0x1020:
                ret = isBattle && scenario.settings.battleType == BATTLE_COIN;
                break;
            case 0x1022:
                ret = mode <= MODE_TIME_TRIAL || mode == MODE_PUBLIC_VS || mode == MODE_PRIVATE_VS;
                break;
            case 0x1024:
                ret = false;
                break;  // Item rain, implement your own check
            default:
                ret = false;
                break;
        }
