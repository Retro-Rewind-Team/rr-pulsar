...
struct FriendRoomCPUSyncPacket {
    ...
    // Two player IDs per byte, low nibble first. CPU slots are contiguous and
    // can be derived from cpuCount, so use this space for the host's race order.
    u8 finishOrder[6];
...
static_assert(sizeof(FriendRoomCPUSyncPacket) == 0xA9, "Friend-room CPU sync packet layout changed");
...
struct PulRH1 : public RKNet::RACEHEADER1Packet {
    ...
    // Friend-room CPU state is part of PulRH1's backing layout, but is only
    // transmitted when friend-room CPU transport is active.
    FriendRoomCPUSyncPacket friendRoomCPUSync;
};
...
static const u32 PulRH1SizeFull = sizeof(PulRH1) - sizeof(FriendRoomCPUSyncPacket);
static const u32 PulRH1SizeBase = PulRH1SizeFull - PulRH1LapKoSize - PulRH1BattleRoyaleSize;
static const u32 PulRH1SizeLapKo = PulRH1SizeBase + PulRH1LapKoSize;
static_assert(sizeof(PulRH1) == 0xFA, "Friend-room CPU RH1 layout changed");
static_assert(sizeof(PulRH1) <= 0xFF, "Friend-room CPU RH1 section is too large");
...
struct PulSELECT : public RKNet::SELECTPacket {
    ...
    // Generated by the friend-room host once per race. Every console uses this
    // packet value to build the same randomized CPU character/kart roster.
    u32 friendRoomCPUSeed;
